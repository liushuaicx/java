                ...
                <div class="box-body no-padding">
                    <table class="table table-hover">
                        ...
                        <c:if test="${empty customerPageInfo.list}">
                            <tr>
                                <td colspan="6">你还没有客户，可以 <a href="/customer/add">新增客户</a></td>
                            </tr>
                        </c:if>
                        <c:forEach items="${customerPageInfo.list}" var="customer">
                            <tr class="detail" rel="${customer.id}">
                                <td><span class="name-avatar" <c:if test="${customer.sex =='女'}"> style="background-color: pink"</c:if>>${customer.custName.subSequence(0,1)}</span></td>
                                <td>${customer.custName}</td>
                                <td>${customer.jobTitle}</td>
                                <td><fmt:formatDate value="${customer.lastContractTime}" pattern="YY-MM-dd HH:mm"/></td>
